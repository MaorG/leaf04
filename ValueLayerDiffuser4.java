...
public class ValueLayerDiffuser4 extends CustumValueLayerDiffuser {
	...
	protected double maxStableDt;

	protected int numOfRepeats;
	protected double dtInRepeat;
	...
	public void setDt(double dt) {
		this.dt = dt;
		determineMaxStableDt();

		this.numOfRepeats = (int) Math.ceil(this.dt / this.maxStableDt);
		this.dtInRepeat = this.dt / (double) this.numOfRepeats;

	}

	@Override
	public void determineMaxStableDt() {
		maxStableDt = 0.5 * dx * dx / diffusionConst;
	}
		...
		if (size == 2) {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					double weightedAvgDeltaDiff = (uE + uW + uN + uS) * 0.25 - oldVal;

					// apply the diffusion and evaporation constants
					double newVal = oldVal +  (dtInRepeat / (dx*dx)) * weightedAvgDeltaDiff * diffusionConst;
...
//		          double weightedAvg = ((uE + uN + uW + uS) * 4 + (uNE + uNW + uSW + uSE)) / 20.0;
	...
	public void diffuse() {
		...
		for (int repeat = 1; repeat < this.numOfRepeats; repeat++)
		{
			computeVals();
			int size = valueLayer.getDimensions().size();
